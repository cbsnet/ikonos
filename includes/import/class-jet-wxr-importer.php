 ...
 * Main importer class
 */
class Jet_WXR_Importer extends WP_Importer {
	/**
	...
	public function __construct( $options = array(), $file = null ) {
		...
		$this->logger = new Jet_Data_Importer_Logger();
	...
	public function prepare_import() {
		...
		jdi_cache()->update( 'total_count', $count );
	...
	public function update_summary() {
		...
		jdi_cache()->update( 'import_summary', $summary );
	...
	public function update_processed_summary( $type ) {
		...
		jdi_cache()->update( 'processed_summary', $processed );
		...
		while ( $this->reader->read() ) {
			...
			switch ( $this->reader->name ) {
				case 'wp:wxr_version':
					...
					jdi_cache()->update( 'version', $version );
				...
				case 'generator':
					jdi_cache()->update( 'generator', $this->reader->readString() );
				...
				case 'blog_description':
					...
					jdi_cache()->update( 'description', $description );
				...
				case 'wp:base_site_url':
					jdi_cache()->update( 'siteurl', $this->reader->readString() );

					$this->next();
					break;

				case 'wp:base_blog_url':
					jdi_cache()->update( 'home', $this->reader->readString() );
	...
	protected function parse_author_node( $node ) {
		...
		$saved = jdi_cache()->update( $data['user_login'], $result, 'users' );
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		$processed_user_slug = jdi_cache()->get( 'user_slug', 'mapping' );
		$processed_terms     = jdi_cache()->get( 'terms', 'mapping' );
		$remap_posts         = jdi_cache()->get( 'posts', 'requires_remapping' );
		...
		if ( $requires_remapping ) {
			...
			jdi_cache()->update( 'posts', $remap_posts, 'requires_remapping' );
		}
		$this->mark_post_exists( $data, $post_id );
		jdi_cache()->update( 'posts', $processed_posts, 'mapping' );
	...
	protected function process_author( $data, $meta ) {
		...
		$login              = $original_slug;
		$user_slug_override = jdi_cache()->get( 'user_slug_override' );
	...
	protected function process_term( $data, $meta ) {
		...
		jdi_cache()->update( 'terms', $processed_terms, 'mapping' );
		jdi_cache()->update( 'term_id', $processed_term_id, 'mapping' );
		jdi_cache()->update( 'term_slug', $processed_term_slug, 'mapping' );
	...
	protected function process_comments( $comments, $post_id, $post, $post_exists = false ) {
		...
		$num_comments       = 0;
		$processed_comments = jdi_cache()->get( 'comments', 'mapping' );
		$processed_users    = jdi_cache()->get( 'users', 'mapping' );
		$remap_comments     = jdi_cache()->get( 'comments', 'requires_remapping' );
		...
		jdi_cache()->update( 'comments', $processed_comments, 'mapping' );
		jdi_cache()->update( 'comments', $remap_comments, 'requires_remapping' );
	...
	protected function process_post_meta( $meta, $post_id, $post ) {
		...
		$remap_meta = jdi_cache()->get( 'posts_meta', 'requires_remapping' );
		...
		foreach ( $meta as $meta_item ) {
			...
			if ( $key ) {
				...
				if ( '_thumbnail_id' == $key ) {
					jdi_cache()->update( $post_id, (int) $value, 'featured_images' );
				}
			}
		}

		jdi_cache()->update( 'posts_meta', $remap_meta, 'requires_remapping' );
	...
	protected function maybe_add_remap_item( $meta_key, $requires_remapping, $object_id, $context ) {
		...
		$remap_meta = jdi()->get_setting( 'remap', $search_key );
	...
	protected function process_menu_item_meta( $post_id, $data, $meta ) {
		...
		$processed_term_id  = jdi_cache()->get( 'term_id', 'mapping' );
		$processed_posts    = jdi_cache()->get( 'posts', 'mapping' );
		$missing_items      = jdi_cache()->get( 'missing_menu_items' );
		$remap_posts        = jdi_cache()->get( 'posts', 'requires_remapping' );
	...
	protected function process_attachment( $post, $meta, $remote_url ) {
		...
		jdi_cache()->update( $remote_url, $upload['url'], 'url_remap' );

		// If we have a HTTPS URL, ensure the HTTP URL gets replaced too
		if ( substr( $remote_url, 0, 8 ) === 'https://') {
			$insecure_url = 'http' . substr( $remote_url, 5 );
			jdi_cache()->update( $insecure_url, $upload['url'], 'url_remap' );
		}
	...
	protected function post_exists( $data ) {
		...
		$existing_posts = jdi_cache()->get( 'posts', 'exists' );
		...
		jdi_cache()->update( 'posts', $existing_posts, 'exists' );
	...
	protected function comment_exists( $data ) {
		...
		$existing_comments = jdi_cache()->get( 'comments', 'exists' );
		...
		jdi_cache()->update( 'comments', $existing_comments, 'exists' );
	...
	protected function term_exists( $data ) {
		...
		$existing_terms = jdi_cache()->get( 'terms', 'exists' );
		...
		jdi_cache()->update( 'terms', $existing_terms, 'exists' );
